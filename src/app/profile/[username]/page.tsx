...
interface Thought {
  ...
  // For now, this matches the modal's selection.
}
...
export default function ProfilePage() {
  ...
  const fetchProfileData = useCallback(
    ...
    [error],
  ) // Removed setError from dependencies as it can cause loops. Check if 'error' is truly needed.
  ...
  useEffect(() => {
    const handleNewThought = (event: Event) => {
      ...
      if (viewedUser && newThoughtFromModal.user_id === viewedUser.id) {
        ...
        console.log(
          `ProfilePage: Received new thought, but user_id ${newThoughtFromModal.user_id} does not match viewedUser.id ${viewedUser.id}`,
        )
      } else {
  ...
  const handleDeleteThought = async (thoughtId: string) => {
    ...
    try {
      const { error } = await supabase.from("thoughts").delete().eq("id", thoughtId).eq("user_id", authUser.id) // Important: ensure only own thoughts can be deleted
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from("avatars").upload(filePath, file, {
        cacheControl: "3600",
        upsert: true, // upsert:true is good for replacing existing avatar if named the same
      })
  ...
  if (loading && !viewedUser) {
    // Modified condition to show loading only if no user data yet
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-gray-400" /> {/* MODIFIED */}
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-4">
        ...
        <button
          ...
          className="mt-4 px-6 py-2 bg-white text-black rounded hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-opacity-75" // MODIFIED
        >
          Try Again
        </button>
    ...
    <div className="max-w-2xl mx-auto px-4 py-4 sm:py-8 text-white">
      ...
      <div className="mb-8 p-4 sm:p-6 bg-gray-900/50 rounded-xl border border-gray-800">
        ...
        <div className="block sm:hidden">
          {/* Mobile: Avatar and Action Button Row */}
          <div className="flex items-center justify-between mb-4">
            <div className="relative group flex-shrink-0">
              ...
              {isOwnProfile && (
                <label
                  htmlFor="avatar-upload-input-mobile"
                  className="absolute inset-0 w-full h-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer rounded-full bg-black bg-opacity-50"
                >
                  <input
                    ...
                    className="hidden"
                  />
                  <div className="p-1.5 rounded-full bg-black bg-opacity-70">
            ...
            <div className="flex-shrink-0">
              {isOwnProfile ? (
                <button
                  ...
                  className="px-4 py-2 bg-white text-gray-900 rounded-full hover:bg-gray-100 transition-colors text-sm font-medium border border-gray-300"
                >
                  Edit Profile
                </button>
              ) : (
                <button
                  ...
                  className={`px-4 py-2 text-white rounded-full transition-colors duration-150 ease-in-out text-sm font-medium border
              ${
                isCurrentlyFollowing
                  ? "bg-transparent border-gray-600 hover:bg-red-600 hover:border-red-600" // Red is kept for unfollow action
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-4 text-sm">
              <div className="flex items-center gap-1">
                <span className="font-semibold text-white">{thoughtCount}</span>
            ...
            {viewedUser.joined_at && typeof viewedUser.joined_at === "string" && (
              <p className="text-xs text-gray-500">
                Joined{" "}
                {new Date(viewedUser.joined_at).toLocaleDateString(undefined, { year: "numeric", month: "long" })}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="hidden sm:flex items-start gap-6">
          ...
          <div className="relative group flex-shrink-0">
            ...
            {isOwnProfile && (
              <label
                htmlFor="avatar-upload-input-desktop"
                className="absolute inset-0 w-full h-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer rounded-full bg-black bg-opacity-50"
              >
                <input
                  ...
                  className="hidden"
                />
                <div className="p-2 rounded-full bg-black bg-opacity-70">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                {viewedUser.bio && (
                  <p className="text-gray-300 text-sm leading-relaxed mb-4 max-w-lg">{viewedUser.bio}</p>
                )}
              ...
              <div className="flex-shrink-0 ml-4">
                {isOwnProfile ? (
                  <button
                    onClick={handleEditToggle}
                    className="px-6 py-2.5 bg-white text-gray-900 rounded-full hover:bg-gray-100 transition-colors text-sm font-medium border border-gray-300"
                  >
                    Edit Profile
                  </button>
                ) : (
                  <button
                    onClick={handleFollowToggle}
                    className={`px-6 py-2.5 text-white rounded-full transition-colors duration-150 ease-in-out text-sm font-medium border
                ${
                  isCurrentlyFollowing
                    ? "bg-transparent border-gray-600 hover:bg-red-600 hover:border-red-600" // Red is kept for unfollow action
                    : "bg-white text-gray-900 border-white hover:bg-gray-100"
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <input
                    ...
                    className="block w-full bg-gray-800 border border-gray-600 rounded-xl px-4 py-3 text-white shadow-sm focus:ring-2 focus:ring-gray-400 focus:border-transparent placeholder-gray-500 transition-all" // MODIFIED
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="block w-full bg-gray-800 border border-gray-600 rounded-xl pl-8 pr-4 py-3 text-white shadow-sm focus:ring-2 focus:ring-gray-400 focus:border-transparent resize-none placeholder-gray-500 transition-all" // MODIFIED
                ...
                <div>
                  <label htmlFor="edit-bio" className="block text-sm font-medium text-gray-300 mb-2">
                    Bio
                  </label>
                  <textarea
                    id="edit-bio"
                    ...
                    rows={4}
                    className="block w-full bg-gray-800 border border-gray-600 rounded-xl px-4 py-3 text-white shadow-sm focus:ring-2 focus:ring-gray-400 focus:border-transparent resize-none placeholder-gray-500 transition-all" // MODIFIED
                    placeholder="Tell us a little about yourself..."
                  />
                  <p className="text-xs text-gray-500 mt-2">{editBio.length}/160 characters</p>
                </div>
              </div>
            </div>
            ...
            <div className="px-6 py-4 bg-gray-800/30 border-t border-gray-700/50">
              <div className="flex justify-end gap-3">
                <button
                  ...
                  className="px-6 py-2.5 bg-transparent border border-gray-600 text-gray-300 rounded-xl hover:bg-gray-700 transition-colors font-medium"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={loading}
                  className="px-6 py-2.5 bg-white text-black rounded-md hover:bg-gray-200 disabled:opacity-50 transition-colors font-medium" // MODIFIED
                >
      ...
      {isOwnProfile && (
        <div className="my-6 sm:my-8 p-4 bg-gray-800 rounded-lg border border-gray-700">
          ...
          <textarea
            ...
            className="w-full p-3 rounded bg-gray-700 text-white resize-none border border-gray-600 focus:ring-2 focus:ring-gray-400 focus:border-transparent placeholder-gray-500 text-sm sm:text-base" // MODIFIED
            rows={3}
            disabled={posting}
          />
          <button
            ...
            className="mt-3 px-4 sm:px-5 py-2 bg-white text-black rounded-md hover:bg-gray-200 disabled:opacity-50 transition-colors font-medium text-sm sm:text-base" // MODIFIED
          >
      ...
      <div className="space-y-4">
        ...
        </h3>
        {
          ...
          ) /* Show loading for thoughts specifically */
        }
        {!loading && thoughts.length === 0 ? (
          ...
          thoughts.map((thought) => (
            <div
              key={thought.id}
              className="bg-gray-800 p-4 rounded-lg shadow text-white relative border border-gray-700 hover:border-gray-600 transition-colors"
            >
              <p className="whitespace-pre-wrap break-words text-gray-100">{thought.content}</p>
              <p className="text-xs text-gray-500 mt-3">
                {new Date(thought.created_at).toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" })}
              </p>
              {isOwnProfile && (
                <button
